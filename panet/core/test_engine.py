...
# from panet.core.rpn_generator import generate_rpn_on_dataset  #TODO: for rpn only case
# from panet.core.rpn_generator import generate_rpn_on_range
from panet.core.test import im_detect_all
from panet.datasets import task_evaluation
from panet.datasets.json_dataset import JsonDataset
from panet.modeling import model_builder
import panet.nn as mynn
from panet.utils.detectron_weight_helper import load_detectron_weight
import panet.utils.env as envu
import panet.utils.net as net_utils
import panet.utils.subprocess as subprocess_utils
import panet.utils.vis as vis_utils
from panet.utils.io import save_object
...
def test_net_on_dataset(
    ...
    # seg_res_file = os.path.join(
    #     output_dir, 'segmentations_' + dataset.name + '_results'
    # )
    # from panet.datasets.json_dataset_evaluator import _w
    # _write_coco_segms_results_file(
    #         dataset, all_boxes, all_segms, seg_res_file
    # )
...
def test_net(
    ...
    for i, entry in enumerate(roidb):
        ...
        cls_boxes_i, cls_segms_i, cls_keyps_i, cls_segms_siim_i = im_detect_all(model, im, box_proposals, timers)
        extend_results(i, all_boxes, cls_boxes_i)
        if cls_segms_i is not None:
            extend_results(i, all_segms, cls_segms_i)

        if cls_segms_siim_i is not None:
            extend_results(i, all_segms, cls_segms_siim_i)
