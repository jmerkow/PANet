...
import panet.utils.boxes as box_utils
import panet.utils.blob as blob_utils
import panet.utils.fpn as fpn_utils
import panet.utils.image as image_utils
import panet.utils.keypoints as keypoint_utils
...
def im_detect_all(model, im, box_proposals=None, timers=None):
    ...
    if cfg.MODEL.MASK_ON and boxes.shape[0] > 0:
        ...
        cls_segms, cls_segms_siim = segm_results(cls_boxes, masks, boxes, im.shape[0], im.shape[1])
        timers['misc_mask'].toc()
    else:
        cls_segms, cls_segms_siim = None, None
    ...
    return cls_boxes, cls_segms, cls_keyps, cls_segms_siim
...
def im_detect_mask(model, im_scale, boxes, blob_conv):
    ...
    # print(pred_masks.min(), pred_masks.max(), len(np.unique(pred_masks)))
...
def segm_results(cls_boxes, masks, ref_boxes, im_h, im_w):
    ...
    cls_segms_siim = [[] for _ in range(num_classes)]
    mask_ind = 0
    ...
    for j in range(1, num_classes):
        ...
        for _ in range(cls_boxes[j].shape[0]):
            ...
            mask_binary = np.array(mask > cfg.MRCNN.THRESH_BINARIZE, dtype=np.uint8)
            ...
            im_mask[y_0:y_1, x_0:x_1] = mask_binary[
            ...
            # rle_siim = {'rle_siim': mask2rle(im_mask*255, im_mask.shape[0], im_mask.shape[1])}
            #For dumping to json, need to decode the byte string.
            # https://github.com/cocodataset/cocoapi/issues/70
            rle['counts'] = rle['counts'].decode('ascii')
            # segms_siim.append(rle_siim)
            segms.append(rle)

            mask_ind += 1

        cls_segms[j] = segms
        cls_segms_siim[j] = segms_siim

    assert mask_ind == masks.shape[0]
    return cls_segms, None
