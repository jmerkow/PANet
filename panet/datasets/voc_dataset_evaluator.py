...
from panet.datasets.dataset_catalog import DATASETS
from panet.datasets.dataset_catalog import DEVKIT_DIR
from panet.datasets.voc_eval import voc_eval
...
def _do_matlab_eval(json_dataset, salt, output_dir='output'):
    ...
    info = voc_info(json_dataset)
    path = os.path.join(
        cfg.ROOT_DIR, 'panet', 'datasets', 'VOCdevkit-matlab-wrapper')
    cmd = 'cd {} && '.format(path)
